...
SELECT cim FROM mu WHERE ev=1916 ORDER BY cim;SELECT nev,tipus FROM mu join kapcsolat ON mu.id=muid join alkoto on alkotoid=alkoto.id WHERE cim="Bob herceg";

SELECT ev FROM mu ORDER by ev DESC limit 1;

SELECT nev,COUNT(muid) FROM alkoto join kapcsolat on alkotoid=alkoto.id GROUP by alkotoid